(() => {
  ...
  function renderStatsCards(stats, session) {
    const cards = [
      ["total", "📦", stats.total, "전체 장비", "controllers.html"],
      ["online", "🟢", stats.online, "온라인", "controllers.html?status=online"],
      ["warning", "🟡", stats.warning + stats.error, "주의/오류", "controllers.html?status_group=issue"],
      ["danger", "🛠️", stats.asUrgent, "AS 임박", "controllers.html?as=urgent"],
      ["info", "🔥", stats.heaterOn, "히터 작동", "controllers.html?heater=on"],
      ["info", "❄️", stats.snowDetected, "눈 감지", "controllers.html?snow=detected"]
    ];

    return `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin-bottom:24px">
        ${cards.map(([klass, icon, value, label, href]) => `
          <button
            type="button"
            class="stat-card ${klass} stat-card-link"
